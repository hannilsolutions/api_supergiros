...
contratos.id_contrato,
        concat(contratos.id_contrato, 'A',clientes.documento) as referencia
         ...
         WHERE contratos.id_servicio IN (1,2,3,4,5,6,7,8,9) AND clientes.tipo_cliente = 'N' AND clientes.documento = :documento AND contratos.estado IN (1,2)";
...
//Funcion para cargar todas las deudas de cada contrato, esta es llamada por una foreach de la funcion getAll
    public function deudasContrato($contrato) {
        $sql = "SELECT 
        deudasb.id_contrato, 
        ...
        WHERE deudasb.id_contrato = :contrato ";
        ...
        try {
            ...
            if($stm->rowCount() > 0){
                 ...
                 $saldo = $resultado->total_debe - $resultado->total_abonos;
                 return $saldo;
            }else{
                return false;
            }
